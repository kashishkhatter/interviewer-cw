...
export async function createInterview(interviewData) {
  try {
    ...
    const requiredFields = ['mockId', 'jsonMockResp', 'jobPosition', 'jobDesc', 'jobExperience', 'createdBy'];
    ...
    console.log('Attempting database insertion with values:', {
      ...
      createdAt: interviewData.createdAt || new Date().toISOString().split('T')[0]
    });
...
export async function getInterviewList(clerkEmail, jwtEmail) {
  try {
    ...
    if (clerkEmail) emailConditions.push(eq(MockInterview.createdBy, clerkEmail));
    if (jwtEmail) emailConditions.push(eq(MockInterview.createdBy, jwtEmail));

    console.log('Querying with conditions:', emailConditions.map(c => c.toString()));
    ...
    console.log('Emails found in database:', emailCheck.map(e => e.email));
...
export async function saveUserAnswer(answerData) {
  try {
    ...
    const resp = await db
      ...
      .values({
        ...answerData,
        createdAt: answerData.createdAt || new Date().toISOString().split('T')[0]
      })
      .execute();
